...
public class ShoppingItemOfferTest
{
	...
	public void assertNoOffer()
	{
		...
		BigDecimal totalForSingleItem = shoppingItemNoOffer.getOfferOnThisItem().calculateTotalForOffer(shoppingItemNoOffer, 1);
		...
		BigDecimal totalForTwoItems = shoppingItemNoOffer.getOfferOnThisItem().calculateTotalForOffer(shoppingItemNoOffer, 2);
	...
	public void assertBOGOFOffer()
	{
		...
		BigDecimal totalForSingleItem = shoppingItemBOGOF.getOfferOnThisItem().calculateTotalForOffer(shoppingItemBOGOF, 1);
		assertEquals("total should be same as price of item", shoppingItemBOGOF.getPrice(), totalForSingleItem);
		
		BigDecimal totalForDoubleItem = shoppingItemBOGOF.getOfferOnThisItem().calculateTotalForOffer(shoppingItemBOGOF, 2);
		assertEquals("total should be same as price of item", shoppingItemBOGOF.getPrice(), totalForDoubleItem);
		
		BigDecimal totalForThreeItems = shoppingItemBOGOF.getOfferOnThisItem().calculateTotalForOffer(shoppingItemBOGOF, 3);
